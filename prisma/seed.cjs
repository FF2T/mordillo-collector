...
// Create user
db.prepare(`INSERT OR IGNORE INTO User (email, name, passwordHash, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)`)
  .run('jhoussiere@yahoo.fr', 'Sébastien', passwordHash, now, now);

const user = db.prepare(`SELECT id FROM User WHERE email = ?`).get('jhoussiere@yahoo.fr');
console.log(`User: id=${user.id}`);

// Create puzzles
const insertPuzzle = db.prepare(`
  INSERT INTO Puzzle (name, publisher, year, reference, rarity, estimatedPrice, dimensions, description, edition, isVerified, flagged, createdAt, updatedAt)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Standard', 1, 0, ?, ?)
`);

const insertMany = db.transaction((items) => {
  for (const p of items) {
    insertPuzzle.run(p.name, p.publisher, p.year, p.reference, p.rarity, p.estimatedPrice, p.dimensions, p.description, now, now);
  }
});
...
// Add to collection
const allPuzzles = db.prepare(`SELECT * FROM Puzzle ORDER BY id`).all();
...
const boxConditions = ['Très bon état', 'Usure normale'];

const insertCollection = db.prepare(`
  INSERT INTO CollectionItem (puzzleId, userId, condition, boxCondition, purchasePrice, purchaseDate, notes, createdAt, updatedAt)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
for (let i = 0; i < ownedIndices.length; i++) {
  ...
  insertCollection.run(
    puzzle.id, user.id, conditions[i], boxConditions[i % 2], prices[i],
    purchaseDate, i === 0 ? 'Trouvé sur eBay, très bon état' : null, now, now
  );
}
console.log(`${ownedIndices.length} collection items created`);

// Alerts
const insertAlert = db.prepare(`INSERT INTO Alert (userId, puzzleId, type, message, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)`);
insertAlert.run(user.id, allPuzzles[3]?.id, 'missing_puzzle', `"${allPuzzles[3]?.name}" a été repéré sur eBay à 19€ !`, now);
insertAlert.run(user.id, allPuzzles[25]?.id, 'rare_listing', `Puzzle ultra-rare "${allPuzzles[25]?.name}" listé sur Vinted !`, now);
insertAlert.run(user.id, allPuzzles[7]?.id, 'good_deal', `"${allPuzzles[7]?.name}" disponible à 20€ (estimé 45€) - Score 8/10`, now);
console.log('Sample alerts created');

// Market listings
const insertListing = db.prepare(`
  INSERT INTO MarketListing (puzzleId, source, title, price, condition, hasOriginalBox, isComplete, listingUrl, isActive, detectedAt)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
`);
insertListing.run(allPuzzles[3]?.id, 'ebay', 'Puzzle Mordillo Crazy Parade 1000 pièces Heye NEUF', 19, 'Neuf', 1, 1, 'https://www.ebay.fr/itm/example1', now);
insertListing.run(allPuzzles[25]?.id, 'vinted', 'Rare puzzle Mordillo The Stage 1000p Heye 1993', 120, 'Occasion', 1, 1, 'https://www.vinted.fr/items/example2', now);
insertListing.run(allPuzzles[7]?.id, 'leboncoin', 'Puzzle Mordillo Lovers 1000 pcs complet', 20, 'Bon état', 0, 1, 'https://www.leboncoin.fr/jeux_jouets/example3', now);
insertListing.run(null, 'ebay', 'Puzzle Mordillo 1000 pièces vintage lot', 45, null, null, null, 'https://www.ebay.fr/itm/example4', now);
insertListing.run(allPuzzles[11]?.id, 'vinted', 'Heye Mordillo Cartoon Classics puzzle 1000', 12, 'Bon état', 1, 1, 'https://www.vinted.fr/items/example5', now);
