// Use relative path to the generated client for Prisma 7
...
async function main() {
  ...
  await prisma.alert.createMany({
    data: [
      {
        userId: user.id,
        puzzleId: allPuzzles[3]?.id,
        type: 'missing_puzzle',
        message: `"${allPuzzles[3]?.name}" a été repéré sur eBay à 19€ !`,
      },
      {
        userId: user.id,
        puzzleId: allPuzzles[25]?.id,
        type: 'rare_listing',
        message: `Puzzle ultra-rare "${allPuzzles[25]?.name}" listé sur Vinted !`,
      },
      {
        userId: user.id,
        puzzleId: allPuzzles[7]?.id,
        type: 'good_deal',
        message: `"${allPuzzles[7]?.name}" disponible à 20€ (estimé 45€) - Score 8/10`,
      },
    ],
  });
  console.log('Sample alerts created');

  // Create some sample market listings
  await prisma.marketListing.createMany({
    data: [
      {
        puzzleId: allPuzzles[3]?.id,
        source: 'ebay',
        title: 'Puzzle Mordillo Crazy Parade 1000 pièces Heye NEUF',
        price: 19,
        condition: 'Neuf',
        hasOriginalBox: true,
        isComplete: true,
        listingUrl: 'https://www.ebay.fr/itm/example1',
      },
      {
        puzzleId: allPuzzles[25]?.id,
        source: 'vinted',
        title: 'Rare puzzle Mordillo The Stage 1000p Heye 1993',
        price: 120,
        condition: 'Occasion',
        hasOriginalBox: true,
        isComplete: true,
        listingUrl: 'https://www.vinted.fr/items/example2',
      },
      {
        puzzleId: allPuzzles[7]?.id,
        source: 'leboncoin',
        title: 'Puzzle Mordillo Lovers 1000 pcs complet',
        price: 20,
        condition: 'Bon état',
        hasOriginalBox: false,
        isComplete: true,
        listingUrl: 'https://www.leboncoin.fr/jeux_jouets/example3',
      },
      {
        source: 'ebay',
        title: 'Puzzle Mordillo 1000 pièces vintage lot',
        price: 45,
        listingUrl: 'https://www.ebay.fr/itm/example4',
      },
      {
        puzzleId: allPuzzles[11]?.id,
        source: 'vinted',
        title: 'Heye Mordillo Cartoon Classics puzzle 1000',
        price: 12,
        condition: 'Bon état',
        hasOriginalBox: true,
        isComplete: true,
        listingUrl: 'https://www.vinted.fr/items/example5',
      },
    ],
  });
